...
export class NotRelevantsComponent implements AfterViewInit {
  ...
  private api = inject(NotRelevantService);
  ...
  openDialog(row?: NotRelevant): void {
    setTimeout(() => {
  ...
  softDelete(row: NotRelevant): void {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      width: '420px',
      disableClose: true,
      data: {
        title: 'Eliminar No Es Relevante',
  ...
  /** Restaurar no relevante eliminada */
  restore(row: NotRelevant): void {
