...
import { ProgramPlansDialogComponent } from './program-plans.dialog';

type FilterState = 'active' | 'deleted' | 'all';

@Component({
  standalone: true,
  selector: 'app-program-plans',
  templateUrl: './program-plans.component.html',
  styleUrls: ['./program-plans.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
    MatTableModule,
...
export class ProgramPlansComponent implements AfterViewInit {
  private readonly resource = 'programPlans';
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[ProgramPlans] Error cargando registros:',
            error,
          );
          ...
          this.showMessage(
            'No fue posible cargar los planes de programa.',
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(ProgramPlansDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
  ...
  softDelete(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Eliminar plan de programa',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'El plan de programa fue eliminado correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramPlans] Error eliminando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible eliminar el plan de programa.',
              ),
  ...
  restore(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Restaurar plan de programa',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'El plan de programa fue restaurado correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramPlans] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar el plan de programa.',
              ),
