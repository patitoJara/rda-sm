...
import { ClosureReasonsDialogComponent } from './closure-reasons.dialog';
...
export class ClosureReasonsComponent implements AfterViewInit {
  private readonly resource = 'closureReasons';
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[ClosureReasons] Error cargando registros:',
            error,
          );
          ...
          this.showMessage(
            'No fue posible cargar las causales de cierre.',
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(ClosureReasonsDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
  ...
  softDelete(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Eliminar causal de cierre',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La causal de cierre fue eliminada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ClosureReasons] Error eliminando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible eliminar la causal de cierre.',
              ),
  ...
  restore(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Restaurar causal de cierre',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La causal de cierre fue restaurada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ClosureReasons] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar la causal de cierre.',
              ),
