...
import { DocumentTypesDialogComponent } from './document-types.dialog';
...
export class DocumentTypesComponent implements AfterViewInit {
  private readonly resource = 'documentTypes';
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[DocumentTypes] Error cargando registros:',
            error,
          );
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(DocumentTypesDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
    ...
    ref.afterClosed().subscribe((confirmed: boolean) => {
      ...
      this.api
        ...
        .subscribe({
          ...
          error: (error) => {
            console.error(
              '[DocumentTypes] Error eliminando registro:',
              error,
            );
    ...
    ref.afterClosed().subscribe((confirmed: boolean) => {
      ...
      this.api
        ...
        .subscribe({
          ...
          error: (error) => {
            console.error(
              '[DocumentTypes] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar el tipo de documento.',
              ),
