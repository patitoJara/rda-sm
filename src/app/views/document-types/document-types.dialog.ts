...
@Component({
  standalone: true,
  selector: 'app-document-types-dialog',
  templateUrl: './document-types.dialog.html',
  styleUrls: ['./document-types.dialog.scss'],
  imports: [
    CommonModule,
    ReactiveFormsModule,
    MatDialogModule,
...
export class DocumentTypesDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private readonly dialogRef: MatDialogRef<DocumentTypesDialogComponent>,
  ...
  save(): void {
    ...
    const value =
      this.form.getRawValue() as DocumentTypeFormValue;
    ...
    request$
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[DocumentTypesDialog] Error guardando:',
            error,
          );
  ...
  private getErrorMessage(error: any): string {
    ...
    if (error?.status === 409) {
      return (
        backendMessage ||
        'El código ya pertenece a otro tipo de documento.'
      );
    }
