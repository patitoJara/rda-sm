...
import { ProgramPopulationsDialogComponent } from './program-populations.dialog';

type FilterState = 'active' | 'deleted' | 'all';

@Component({
  standalone: true,
  selector: 'app-program-populations',
  templateUrl: './program-populations.component.html',
  styleUrls: ['./program-populations.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
    MatTableModule,
...
export class ProgramPopulationsComponent implements AfterViewInit {
  private readonly resource = 'programPopulations';
  ...
  private readonly api = inject(CatalogMaintainerService);
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[ProgramPopulations] Error cargando registros:',
            error,
          );
          ...
          this.showMessage(
            'No fue posible cargar las poblaciones de programa.',
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(ProgramPopulationsDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
  ...
  softDelete(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Eliminar población de programa',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La población de programa fue eliminada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramPopulations] Error eliminando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible eliminar la población de programa.',
              ),
  ...
  restore(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Restaurar población de programa',
        message:
          `¿Seguro que deseas restaurar “${row.name}” ` +
          `(ID: ${row.id})?`,
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La población de programa fue restaurada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramPopulations] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar la población de programa.',
              ),
