...
import { RegionsDialogComponent } from './regions.dialog';
...
export class RegionsComponent implements AfterViewInit {
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[Regions] Error cargando registros:',
            error,
          );
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(RegionsDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
    ...
    ref.afterClosed().subscribe((confirmed: boolean) => {
      ...
      this.api
        ...
        .subscribe({
          ...
          error: (error) => {
            console.error(
              '[Regions] Error eliminando registro:',
              error,
            );
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La región fue restaurada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[Regions] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar la región.',
              ),
