...
import { ProgramModalitiesDialogComponent } from './program-modalities.dialog';

type FilterState = 'active' | 'deleted' | 'all';

@Component({
  standalone: true,
  selector: 'app-program-modalities',
  templateUrl: './program-modalities.component.html',
  styleUrls: ['./program-modalities.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
    MatTableModule,
...
export class ProgramModalitiesComponent implements AfterViewInit {
  private readonly resource = 'programModalities';
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            '[ProgramModalities] Error cargando registros:',
            error,
          );
          ...
          this.showMessage(
            'No fue posible cargar las modalidades de programa.',
  ...
  openDialog(row?: CatalogItem): void {
    const ref = this.dialog.open(ProgramModalitiesDialogComponent, {
      width: '560px',
      maxWidth: '95vw',
      disableClose: true,
  ...
  softDelete(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Eliminar modalidad de programa',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La modalidad de programa fue eliminada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramModalities] Error eliminando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible eliminar la modalidad de programa.',
              ),
  ...
  restore(row: CatalogItem): void {
    ...
    const ref = this.dialog.open(ConfirmDialogYesNoComponent, {
      ...
      backdropClass: 'app-backdrop',
      data: {
        title: 'Restaurar modalidad de programa',
        message:
      ...
      this.api
        ...
        .subscribe({
          next: () => {
            ...
            this.showMessage(
              'La modalidad de programa fue restaurada correctamente.',
              'Operación realizada',
              'check_circle',
            );
          },
          ...
          error: (error) => {
            console.error(
              '[ProgramModalities] Error restaurando registro:',
              error,
            );
            ...
            this.showMessage(
              this.getErrorMessage(
                error,
                'No fue posible restaurar la modalidad de programa.',
              ),
